 <?php 
	...
	$select="select * from media_travel where media_id=$id";
              ...
              if($_FILES["image"]["error"]>0)
                ...
                if($_POST["taxonomy_id"]=="..")
                     ...
                     else
                     {
                      ...
                      {
                          $insert_id=$vsc->update('media_travel',$data,"media_id=$id");
                          // upload tam hinh 
                          move_uploaded_file($_FILES["image"]["tmp_name"],"../library/image/".$_FILES["image"]["name"]);
                          // Check Inert
                          if($insert_id > 0)
                          { 
                              $messege="<p class='text-seccess'>The Media-Travel was updated database successfuly</p>";
                          }
                          else
                          {
                             $messege="<p class='text-warning'> The Media-Travel not updated database successfuly!</p>";
                          }
                      }
           }
...
<section>
				<div id="content_admin" class="col-md-10 col-xs-8" >
         			<h2 class="title_content_admin">Edit Media-Travel</h2>
              <?php 
                  if(isset($messege))
                     echo $messege;
               ?>
         		 ...
         		 <div id="content_admin2" class="col-md-10 col-xs-8" style="margin-top:5px; ">
         		 		 <form role="form"  method="post" name="edit_media" enctype="multipart/form-data">
         		 		 		 <div class="form-group">
                                <div class="form-group">
                                    <label >Media-Title</label>
                                    <input type="text" class="form-control" name="media-title" placeholder="" value="<?php if(isset($data['media_title'])) echo htmlentities($data['media_title'],ENT_COMPAT,'UTF-8'); ?>" required>
                                  </div>
                                   <div class="form-group ">
                                   ...
                                   <div class="form-group">
                                      ...
                                      <?php if(isset($messege2)) echo $messege2;?>
                                        ...
                                        <script type="text/javascript">
                                          for (var i = 0; i <edit_media.taxonomy_id.length ; i++) {
                                              if(edit_media.taxonomy_id[i].value=="<?php echo $data['taxonomy_id'] ?>"){
                                                edit_media.taxonomy_id[i].selected=true;
                                              }
                                            }
                                          </script>
                                  </div>
